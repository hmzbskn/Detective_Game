...
public class OyuncuDNAToplamaKontrolcusu : MonoBehaviour
{
    ...
    private void DNAToplamayiDene()
    {
        ...
        if (!bakilanNokta.ToplanabilirMi())
            return;

        ItemData dnaDelili = bakilanNokta.DNADeliliItemData;
        ...
        bool degisimBasarili = hotbarSistemi.AktifHotbarIteminiDegistir(
        ...
        bakilanNokta.ToplandiOlarakIsaretle();

        if (debugMesajlari)
        {
            Debug.Log($"DNA toplandı. Temiz kulak çöpü artık delil oldu: {dnaDelili.ItemAdi}");
        }
